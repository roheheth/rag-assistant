...
def _render_sources(sources: list) -> None:
    """
    Render a structured Sources block from server-verified metadata.

    Sources are NEVER derived from the LLM's text output — they come from
    the 'sources' SSE event which is built server-side from retrieved
    chunk metadata (document_name, page_number, effective_date, doc_status).

    Each source renders as an expander with:
      - Index badge matching the [N] inline marker in the answer
      - Document name, page, effective date, status
    ...
    st.markdown("---")
    st.markdown("**📚 Verified Sources** *(server-verified — not generated by LLM)*")

    for src in sources:
        idx   = src.get("index", "?")
        name  = src.get("document_name", "Unknown Document")
        page  = src.get("page_number")
        date  = src.get("effective_date") or "—"
        status = src.get("doc_status", "Active")
        score  = src.get("relevance_score", 0.0)
        snippet = src.get("snippet", "")

        page_str = f"p. {page}" if page else "p. N/A"
        label = f"[{idx}] {name} · {page_str}"

        with st.expander(label, expanded=False):
            col1, col2, col3 = st.columns(3)
            col1.metric("Effective Date", date)
            col2.metric("Status", status)
            col3.metric("Relevance", f"{score:.2%}")
            if snippet:
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                # Protocol (events arrive in this order):
                #   metadata  — {type, chat_id, turn_number}
                #   chunk × N — {type, content}
                #   sources   — {type, sources: [...]}   ← server-built citations
                #   done      — {type}

                def _token_generator(response):
                    """
                    Yields answer tokens for st.write_stream().
                    Captures chat_id, and verified source list as side-effects
                    stored in the enclosing scope via a mutable list.
                    """
                    for line in response.iter_lines():
                        ...
                        if evt == "metadata":
                            ...
                            # Store so we can render AFTER write_stream returns
                            _captured_sources.append(data.get("sources", []))

                        # 'done' is silently consumed — it just ends the loop

                _captured_sources: list = []

                message_placeholder.empty()
                response_text = st.write_stream(_token_generator(res))

                # ── Render server-verified sources below the answer ───────
                verified_sources = _captured_sources[0] if _captured_sources else []
                with sources_placeholder.container():
                    _render_sources(verified_sources)
